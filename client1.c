...
//If client finds a match for key, it requests Server 1 for value 
  ...
  {
  printf("Client 1 has received a request with search key word %s which maps to key %s", buff1, key[i]);
  printf("\n");
  strcat(msg,key[i]); 
  msg[strlen(msg)] = 0;
//Sends request to server 1 -- rutgers reused code
  ...
  printf("Client 1 sends the request %s to Server 1 with Port number 21328 and IP address ",msg);
  ...
  printf("Client 1's Port number is %d and the IP address is ",port1);
...
//Extracts value from received message and prints value
  if (recvlen > 0) 
  {
    ...
    printf("Client 1 received the value %s from Server 1 with Port number 21328 and IP address ", out);
    puts(inet_ntop(AF_INET, &(servaddr.sin_addr), ip4, INET_ADDRSTRLEN));
    printf("\n");
    printf("Client 1's Port number is %d and the IP address is ",port1);
    puts(inet_ntop(AF_INET, &(client.sin_addr), ip4, INET_ADDRSTRLEN));
